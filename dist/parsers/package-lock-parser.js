    ...
    getDepMap(lockfile) {
        ...
        const flattenLockfileRec = (lockfileDeps, path) => {
            for (const [depName, dep] of Object.entries(lockfileDeps)) {
                const depNode = Object.assign(Object.assign({ labels: {
                        scope: dep.dev ? Scope.dev : Scope.prod,
                    }, name: depName, requires: [], version: dep.version }, (dep.resolved && { resolved: dep.resolved })), (dep.integrity && { integrity: dep.integrity }));
                if (dep.requires) {

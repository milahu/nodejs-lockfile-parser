...
export const extractNameAndIdentifier = (candidate) => {
    let name, identifier;
...
// in dependencies.
export const extractCorrectIdentifierBySemver = (possibleMatches, versionToMatch) => {
    const { name: nameToMatch, identifier: identifierToMatch } = extractNameAndIdentifier(versionToMatch);
    ...
    const match = possibleMatches
        ...
        .filter((name) => semver.satisfies(extractNameAndIdentifier(name).identifier, identifierToMatch))
        .map((name) => ({
        name,
